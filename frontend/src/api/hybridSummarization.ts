/**
 * API client for Hybrid Summarization endpoints
 */
...
export interface MultiDocumentRequest {
...
class HybridSummarizationAPI {
   ...
   */
  async createMultiDocumentSummary(request: MultiDocumentRequest): Promise<SummaryResult> {
    const response = await this.request<SummaryResult>('/multi-document', {
      method: 'POST',
      body: JSON.stringify(request),
    });
    return response.data;
   ...
   */
  async createBatchSummaries(requests: SummarizationRequest[]): Promise<{
    results: Array<{
      index: number;
   ...
   */
  async setUserPreferences(userId: string, preferences: UserPreferences): Promise<{
    user_id: string;
