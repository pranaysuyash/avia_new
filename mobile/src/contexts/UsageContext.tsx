/**
 * Usage and Subscription Context for React Native
...
export function UsageProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshUsage = useCallback(async () => {
    try {
      ...
      // Handle quota and feature errors specifically
      if (error.isQuotaError) {
        handleQuotaError(error);
      } else if (error.isFeatureError) {
  ...
  const clearError = useCallback(() => {
    ...
    dispatch({ type: 'SET_QUOTA_EXCEEDED', payload: false });
    dispatch({ type: 'SET_FEATURE_BLOCKED', payload: null });
  }, []);
...
// Utility function to format usage numbers
export function formatUsage(value: number | string): string {
...
// Utility function to get usage color based on percentage
