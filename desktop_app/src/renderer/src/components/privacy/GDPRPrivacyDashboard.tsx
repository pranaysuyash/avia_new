...
export const GDPRPrivacyDashboard: React.FC<GDPRPrivacyDashboardProps> = ({ className }) => {
  ...
  const saveExportToDesktop = async (exportRequest: ExportRequest) => {
    if (!window.electronAPI || !exportRequest.download_url) return;

    try {
      const result = await window.electronAPI.saveDialog({
        title: 'Save GDPR Export',
        defaultPath: `gdpr_export_${exportRequest.request_id}.${exportRequest.format_type || 'json'}`,
        filters: [
      ...
      if (!result.canceled && result.filePath) {
        // Download file to chosen location
        const response = await fetch(exportRequest.download_url);
        const blob = await response.blob();
        const buffer = await blob.arrayBuffer();
        
        await window.electronAPI.saveFile(result.filePath, Buffer.from(buffer));
        
        showDesktopNotification(
          'Export Saved',
          `GDPR export saved to ${result.filePath}`
        );
        
        // Offer to show in file explorer
        const showFile = await window.electronAPI.showMessageBox({
          type: 'info',
          title: 'Export Saved',
          message: 'Would you like to open the file location?',
          buttons: ['Yes', 'No']
        });
        
        if (showFile.response === 0) {
          await openFileLocation(result.filePath);
        }
      }
    } catch (err) {
      setError('Failed to save export file');
  ...
  const withdrawConsent = async (consentId: string) => {
    ...
    try {
      ...
      showDesktopNotification('Consent Withdrawn', 'Consent withdrawn successfully');
      await fetchConsents();
    } catch (err) {
    ...
    if (!deletionConfirm.confirmation || !deletionConfirm.understanding) {
      if (window.electronAPI) {
        await window.electronAPI.showMessageBox({
          ...
          message: 'Please confirm both checkboxes to proceed with data deletion.'
        });
      } else {
  ...
  const downloadExport = async (exportRequest: ExportRequest) => {
    ...
    // Use desktop-specific save dialog if available
    if (window.electronAPI) {
      await saveExportToDesktop(exportRequest);
    } else {
      // Fallback to browser download
      try {
        const token = 'verification_token_here';
                        ...
                        <TableCell>
                          {exportReq.status === 'completed' && exportReq.download_url ? (
                            <div className="flex gap-1">
                              ...
                              {window.electronAPI && (
                                <Button
                                  onClick={() => window.electronAPI?.openExternal(exportReq.download_url!)}
                                  size="sm"
                                  variant="outline"
                                >
      ...
      <Dialog open={showRightsDialog} onOpenChange={setShowRightsDialog}>
        <DialogContent className="max-w-2xl">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => {
              if (window.electronAPI) {
                window.electronAPI.openExternal('https://gdpr.eu/');
              } else {
                window.open('https://gdpr.eu/', '_blank');
              }
            }}>
