...
// Electron-specific imports for file operations
declare global {
  interface Window {
    electron?: {
      dialog: {
...
const AudioPreprocessingDesktop: React.FC = () => {
  ...
  const selectAudioFromSystem = async () => {
  ...
  // Save processed audio to system
  const saveProcessedAudio = async () => {
    if (!processedAudio || !window.electron?.dialog) return;

    try {
      const result = await window.electron.dialog.showSaveDialog({
        defaultPath: 'processed_audio.wav',
        filters: [
      ...
      if (!result.canceled && result.filePath) {
        ...
        const base64Data = processedAudio.replace(/^data:audio\/[^;]+;base64,/, '');
        const buffer = Buffer.from(base64Data, 'base64');
        
        await window.electron.fs.writeFile(result.filePath, buffer);
  ...
  return (
    <Box sx={{ p: 3, height: '100vh', overflow: 'auto' }}>
      <Typography variant="h4" gutterBottom>
        Advanced Audio Preprocessing - Desktop
      </Typography>
            ...
            <CardContent>
              ...
              <Box
                ...
                sx={{
                  ...
                  textAlign: 'center',
                  cursor: 'pointer',
                  backgroundColor: isDragActive || dragOver ? 'action.hover' : 'transparent',
                  mb: 2,
              ...
              >
                ...
                <Typography variant="caption" color="textSecondary">
                  Supports: WAV, MP3, FLAC, M4A, OGG, AAC (Max 50MB)
                </Typography>
              </Box>

              {/* System File Browser Button */}
              <Button
                variant="outlined"
                startIcon={<FolderOpen />}
                onClick={selectAudioFromSystem}
                fullWidth
                sx={{ mb: 2 }}
              >
                Browse System Files
              ...
              {selectedFilePath && (
                <Paper sx={{ p: 2, mb: 2 }}>
                  ...
                  <Typography variant="body2" color="textSecondary">
                    {window.electron?.path.basename(selectedFilePath) || selectedFilePath}
                  </Typography>
                  ...
                  <Box sx={{ mt: 2, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                    ...
                    >
                      {isPlaying && currentAudio === 'original' ? 'Pause' : 'Play'}
                    </Button>
                    <Button
                      size="small"
                      startIcon={<Stop />}
              ...
              <CardContent>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                  ...
                  <Box>
                    ...
                    <IconButton onClick={saveProcessedAudio} disabled={!processedAudio}>
                      <FolderOpen />
                    </IconButton>
...
export default AudioPreprocessingDesktop;