...
class ActionItemExtractor:
    ...
    def _enhance_meeting_insights(
        ...
        # Extract meeting context and sentiment from transcript
        if transcript:
            # Get overall sentiment of the meeting
            insights.meeting_sentiment = self._analyze_meeting_sentiment(transcript)
            
            # Extract key topics discussed
            insights.key_topics = self._extract_key_topics(transcript)
            
            # Identify meeting tone (formal, informal, technical, etc.)
            insights.meeting_tone = self._analyze_meeting_tone(transcript)
        
        return insights
    
    def _analyze_meeting_sentiment(self, transcript: str) -> str:
        """Analyze overall sentiment of the meeting"""
        # Simple sentiment analysis based on keyword presence
        positive_words = ['good', 'great', 'excellent', 'positive', 'agree', 'success', 'well']
        negative_words = ['bad', 'poor', 'negative', 'disagree', 'problem', 'issue', 'concern']
    ...
    def _extract_key_topics(self, transcript: str) -> List[str]:
        ...
        # Simple topic extraction based on common business topics
        topic_keywords = {
    ...
    def _analyze_meeting_tone(self, transcript: str) -> str:
        """Analyze the tone of the meeting"""
        # Simple tone analysis based on language patterns
        formal_indicators = ['therefore', 'consequently', 'furthermore', 'moreover', 'however']
        technical_indicators = ['api', 'function', 'algorithm', 'database', 'server', 'implementation']
        
        transcript_lower = transcript.lower()
        formal_count = sum(1 for word in formal_indicators if word in transcript_lower)
        technical_count = sum(1 for word in technical_indicators if word in transcript_lower)
