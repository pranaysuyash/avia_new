...
from enhanced_components_refactored import (
    ...
    render_page_frame, log_ux_event
)
...
def main():
    ...
    try:
        ...
        if USER_AUTH_AVAILABLE:
            cu = get_current_user()
            role = getattr(cu, 'role', role) if cu else role
        render_page_frame(
            title="🎤 Audio/Video Transcription & Analysis",
            subtitle="Transcribe, search, analyze, and export with confidence",
            breadcrumb=["App"],
    ...
    # Log screen view
    try:
        log_ux_event("screen_view", screen="app_main")
    except Exception:
        pass
...
def render_simplified_sidebar():
    ...
    with st.sidebar.expander("🔬 Analysis & Processing", expanded=True):
        # Sync analysis mode from query param
        try:
            qp = st.experimental_get_query_params() or {}
            qp_mode = qp.get("app_analysis", [None])[0]
            allowed = ["Basic (spaCy)", "Advanced (OpenAI)", "Advanced+ (Speaker Diarization)", "🌍 Multi-Language"]
            if qp_mode in allowed:
                st.session_state["analysis_mode"] = qp_mode
        except Exception:
            pass
        ...
        # Update query param when changed
        try:
            current = st.experimental_get_query_params() or {}
            if current.get("app_analysis", [None])[0] != analysis_mode:
                current["app_analysis"] = [analysis_mode]
